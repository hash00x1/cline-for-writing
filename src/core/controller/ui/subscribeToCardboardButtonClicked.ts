...
/**
 * Subscribe to cardboard button clicked events
 * @param controller The controller instance
 * @param request The request with provider type
 * @param responseStream The streaming response handler
 * @param requestId The ID of the request (passed by the gRPC handler)
 */
export async function subscribeToCardboardButtonClicked(
	...
	console.log(`[DEBUG] set up cardboard button subscription for ${WebviewProviderType[providerType]} webview`)
	...
	if (requestId) {
		getRequestRegistry().registerRequest(
			requestId,
			cleanup,
			{ type: "cardboard_button_clicked_subscription" },
			responseStream,
		)
	}
}
...
/**
 * Send a cardboard button clicked event to active subscribers of matching provider type
 * @param webviewType The type of webview that triggered the event
 */
export async function sendCardboardButtonClickedEvent(webviewType?: WebviewProviderType): Promise<void> {
	...
	const promises = Array.from(subscriptions.entries()).map(async ([responseStream, providerType]) => {
		...
		try {
			...
			console.error(`Error sending cardboard button clicked event to ${WebviewProviderType[providerType]}:`, error)
			subscriptions.delete(responseStream)
		}
	})
